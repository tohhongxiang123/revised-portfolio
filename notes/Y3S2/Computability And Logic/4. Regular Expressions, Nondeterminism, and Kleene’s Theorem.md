...
To define $\delta^*(q, x\sigma)$ where $x \in \Sigma^*$ and $\sigma \in \Sigma$, we start by considering $\delta^*(q, x\sigma)$ to be an ordinary FA. This is now a *set* of states, and for each state $p$ in this set, $\delta(p, \sigma)$ is also a set. To include all possibilities, we consider
...
The FA $M_1$ can be defined as follows using the *subset construction*: The states of $M_1$ are sets of states of $M$: 

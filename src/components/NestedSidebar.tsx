...
function NestedDirectoryItem({ item }: { item: NestedDirectoryStructure }) {
    ...
    const isItemActive =
        decodeURI(path) === "/" + item.fullPath.replaceAll("\\", "/");

    if (item.children.length === 0) {
        return (
            <Link
                href={`/${item.fullPath}`}
                className={`relative flex w-full cursor-pointer items-center justify-between rounded-md p-2 text-left text-sm hover:bg-gray-500/30 ${
